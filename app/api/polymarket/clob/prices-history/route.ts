...
// Rate limiting: track last request time per market+interval+fidelity
const lastRequestTime = new Map<string, number>();
const MIN_REQUEST_INTERVAL_MS = 2000; // Minimum 2 seconds between requests for same params
...
// Valid Polymarket CLOB intervals (as per API docs)
const VALID_INTERVALS = ['max', '1w', '1d', '6h', '1h'] as const;
type ValidInterval = typeof VALID_INTERVALS[number];

export async function GET(request: NextRequest) {
  const searchParams = request.nextUrl.searchParams;
  const market = searchParams.get('market'); // token_id
  ...
  // Validate interval - only allow valid Polymarket intervals
  if (!VALID_INTERVALS.includes(intervalParam as ValidInterval)) {
    return NextResponse.json(
      {
        ok: false,
        error: 'Invalid interval',
        allowed: VALID_INTERVALS,
        received: intervalParam,
        diagnostics: {
          nodeVersion: process.version,
          nodePlatform: process.platform,
          timestamp: new Date().toISOString(),
  ...
  const cacheKey = `${market}:${interval}:${fidelity}`;
  ...
  try {
    const historyUrl = `${CLOB_API_BASE}/prices-history?market=${encodeURIComponent(market)}&interval=${encodeURIComponent(interval)}&fidelity=${encodeURIComponent(fidelity)}`;
    const { response, error: fetchError } = await robustFetch(historyUrl);
    ...
    if (fetchError || !response) {
      ...
      console.error('[Polymarket CLOB Proxy] Prices-history fetch failed:', JSON.stringify(errorDetails, null, 2));
    ...
    if (!response.ok) {
      ...
      const errorDetails: any = {
        ok: false,
        error: response.status === 404 ? 'History not found' : 'Upstream error',
        market,
        interval,
        fidelity,
        upstreamUrl: historyUrl,
